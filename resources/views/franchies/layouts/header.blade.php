<div class="navbar-header">
    <a class="navbar-brand" href="<?php echo URL::to('/franchies/dashboard'); ?>">
        <b>
...
<div class="navbar-collapse">
    ...
    <ul class="navbar-nav my-lg-0">
        ...
        <li class="nav-item dropdown">
            @php
                $userId = session('user');
                $user = \App\Models\Franchise::find($userId->id);
            @endphp
            <a class="nav-link dropdown-toggle text-muted waves-effect waves-dark" href="" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                @if($user->image)
                    <img src="{{ asset('images/franchies/profile/' .$user->image) }}" alt="user" class="profile-pic">
                @else
                ...
                <ul class="dropdown-user">
                    <li>
                        ...
                        <div class="dw-user-box">
                            <a href="{{ route('franchies.users.profile') }}">
                                <div class="u-img">
                                    @if($user->image)
                                        <img src="{{ asset('images/franchies/profile/' .$user->image) }}" alt="user" style="max-width: 45px;">
                                    @else
                    ...
                    <li><a href="{{ route('franchies.users.password')}}" >
                            Change Password
                        </a></li>
                    <li role="separator" class="divider"></li>
                   <li>
                        <a href="{{ route('franchies.logout') }}"
                           onclick="event.preventDefault();
                                         document.getElementById('logout-form').submit();">
                            Logout
                        </a>
                        <form id="logout-form" action="{{ route('franchies.logout') }}" method="POST" style="display: none;">
                            @csrf
                        </form>
                    </li>

                    ...
                    <div class="card-header">
                        <ul class="nav nav-tabs align-items-end card-header-tabs w-100 justify-content-center">
                            <li class="nav-item" style="border: 1px solid #ff683a;border-radius:2px">
                                <a class="nav-link active" href="{!! route('admin.drivers') !!}"><i class="fa fa-list mr-2"></i>{{trans('lang.driver_table')}}</a>
                            </li>
                            <li class="nav-item" style="border: 1px solid #ff683a;border-radius:2px">
                                <a class="nav-link" href="{!! route('admin.drivers.create') !!}"><i class="fa fa-plus mr-2"></i>{{trans('lang.drivers_create')}}</a>
                            </li>
                        </ul>
                    </div>
                        ...
                        <div class="table-responsive m-t-10">
                            ...
                            <table id="data-table" class="display nowrap table table-hover table-striped table-bordered table table-striped" cellspacing="0" width="100%">
                                ...
                                <tr>
                                    ...
                                    <td>{{$d->first_name??$d->first_name}}&nbsp;{{$d->last_name??$d->last_name}}</td>
                                    <td>{{$d->phone}}</td>
                                    @if(isset($d->team->franchies_id))
                                        <?php
                                            $franchise = \App\Models\Franchise::find($d->team->franchies_id);
                                            $franchiseName = $franchise ? $franchise->franchies_name : 'N/A';
                                        ?>
                                        <td class="franchise-name" data-franchise-id="{{ $d->team->franchies_id }}" style="cursor:pointer;" data-franchise-name="{{ $franchiseName }}">
                                            {{ $franchiseName }}
                                        </td>
                                    @else
                                        <td class="franchise-name" data-franchise-id="{{ $d->team->franchies_id ?? '' }}" style="cursor:pointer;">N/A</td>
                                    @endif
                                     <td class="employee-name" data-employee-id="{{ $d->team->employee_id ?? '' }}"
                                    data-employee-name="{{ $d->team && $d->team->employee_id ? App\Models\Employee::find($d->team->employee_id) : 'N/A' }}" style="cursor:pointer;">
                                    ...
                                    <td>
                                        <a href="{{ url('admin/drivers/edit/' . $d->id) }}" class="btn btn-primary btn-sm"> <i class="fa fa-edit"></i> </a>
                                        <a href="{{ url('admin/drivers/view/' . $d->id) }}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>
                                        <!--<a href="{{ url('admin/drivers/delete/' . $d->id) }}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure wants to delete');"><i class="fa fa-trash"></i></a>-->
                                        <a href="#" class="btn btn-sm btn-danger"
                                           onclick="showConfirmation('Are you sure wants to delete?', function() { window.location.href = '{{ url('admin/drivers/delete/' . $d->id) }}'; })">
    ...
    $(document).ready(function () {
        ...
        $('.franchise-name').on('click', function () {
            ...
            if (franchiesName !== 'N/A') {
                ...
                $.ajax({
                    url: '/admin/drivers/showFranchiesDetails/' + franchiesId,
                    type: 'GET',
                    success: function (data) {

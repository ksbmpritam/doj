...
<div class="page-wrapper">
    <form action="{{ url('admin/promoCode/store_restaurant/'. $promo_code->id)}}" method="post" enctype="multipart/form-data">
        @csrf
...
$(document).ready(function () {
    ...
    function displayAllRestaurant(data) {
        ...
        data.forEach(function (restaurant) {
            ...
            $('#restaurantList').append(`
                <input type="checkbox" class="coupon_enabled" id="restaurant_${restaurant.id}" name="restaurant_id[]" value="${restaurant.id}"  ${isChecked}>
                <label class="m-2 control-label user-label" for="restaurant_${restaurant.id}">${restaurant.name}</label>
            `);
    ...
    $.ajax({
        url: '/admin/promoCode/get_restaurant',
        type: 'GET',
        data: {
            id: "<?php echo $id ?>"
        },
    ...
    $('#restaurantSearch').on('keyup', function () {
        ...
        showLoader();
          $('#restaurantList').empty();
        $.ajax({
            url: '/admin/promoCode/search_restaurant',
            type: 'GET',
            data: {
                search: searchValue,
                id: "<?php echo $id ?>"

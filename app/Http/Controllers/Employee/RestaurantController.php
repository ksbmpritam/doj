...
class RestaurantController extends Controller
{
    ...
    public function insert(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'ifsc_code' => 'nullable|regex:/^[A-Za-z]{4}\d{7}$/i',
            'address' => 'required',
            'latitude' => 'required',
            'longitude' => 'required',
    ...
    public function view($id)
    {
        ...
        return view('employee.team.detail.view', compact('restaurant','team_id'));
    }
    ...
    public function restaurantDetail($id)
    {
        try {
            ...
            return view('employee.team.detail.restaurant_detail', compact('restaurant','team_id', 'categories', 'restaurant_admin', 'workingHours', 'specialOffers', 'galleryImages','dineImages','days'));
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $exception) {
        }
    }
    ...
    public function approvalEdit($id){
        $restaurant = Restaurant::findOrFail($id);
        $team_id = $restaurant->team_id;
        return view('employee.team.detail.update_status', compact('restaurant','team_id'));
    }
    ...
    public function approvalUpdate($id, Request $request){
        ...
        return redirect()->route('employee.team.restaurant', ['id' => $restaurant->team_id])->with('success', 'Restaurant status updated successfully.');
    }
    ...
    public function fracnchies_restaurant($id)
    {
        $team_ids =$id;
        $restaurant = Restaurant::where('team_id',$id)->orderBy('id', 'desc')->get();
        return view('employee.franchies.detail.franchies.restaurant_list', compact('restaurant','team_ids'));
    }
    
    public function fracnchiesRestaurantEdit($id)
    {
        try {
            $restaurant = Restaurant::findOrFail($id);
            ...
            return view('employee.franchies.detail.franchies.restaurant_edit_detail', compact('restaurant', 'team_ids', 'categories', 'restaurant_admin', 'workingHours', 'specialOffers', 'galleryImages', 'dineImages', 'days'));
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $exception) {
            return abort(404); // Return a 404 response
        }
    }


    public function fracnchiesRestaurantDetail($id)
    {
        try {
            $restaurant = Restaurant::findOrFail($id);
            $team_ids = $restaurant->team_id;
            ...
            return view('employee.franchies.detail.franchies.restaurant_detail', compact('restaurant','team_ids', 'categories', 'restaurant_admin', 'workingHours', 'specialOffers', 'galleryImages','dineImages','days'));
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $exception) {
        }
    }
    
    public function fracnchiesRestaurantUpdate($id, Request $request) {
        $validator = Validator::make($request->all(), [
        ...
        $admin->approved_by = "emp_" . Session::get('user')->name;
        $admin->approved_by_id = Session::get('user')->id;
        $admin->approved_date = Carbon::now()->format('Y/m/d');
        $admin->cancel_reason = $cancel_reason;
        $admin->save();
    
        return redirect()->route('employee.franchies.team.restaurant', $admin->id)->with('success', 'Restaurant updated successfully.');
    }

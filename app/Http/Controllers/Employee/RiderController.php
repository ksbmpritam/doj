...
class RiderController extends Controller
{
    ...
    public function riderList($id)
    {
        ...
        return view('employee.team.detail.rider_list', compact('drivers', 'driver','team_id'));
    }
    ...
    public function riderDetail($id)
    {
        try {
            $team_id = $id;
            $driver = Driver::findOrFail($id);
            return view('employee.team.detail.rider_detail', compact('driver','team_id'))->with('id', $id);
    ...
    public function approvalEdit($id){
        $team_id = $id;
        $driver = Driver::findOrFail($id);
        return view('employee.team.detail.update_rider_status', compact('driver','team_id'));
    }
    ...
    public function approvalUpdate($id, Request $request){
        ...
        return redirect()->route('employee.team.rider', ['id' => $driver->team_id])->with('success', 'Restaurant status updated successfully.');
    }
    
    
    public function franchiesRiderList($id)
    {
        $team_ids = $id;

        $driver = Driver::where('team_id', $id)->orderBy('id', 'desc')->get();
        return view('employee.franchies.detail.franchies.rider_list', compact('driver','team_ids'));
    }

    public function franchiesRiderDetail($id)
    {
        try {
            $driver = Driver::findOrFail($id);
            $team_ids = $driver->team_id;
            return view('employee.franchies.detail.franchies.rider_detail', compact('driver','team_ids'))->with('id', $id);
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $exception) {
        }
    }
    
    public function fracnchiesRiderEdit($id)
    {
        try {
            $driver = Driver::findOrFail($id);
            $team_ids = $driver->team_id;
            return view('employee.franchies.detail.franchies.rider_edit', compact('driver','team_ids'))->with('id', $id);
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $exception) {
        }
    }
    
    public function fracnchiesRiderUpdate($id, Request $request) {
        ...
        $Driver->approved_by = "emp_" . Session::get('user')->name;
        $Driver->approved_by_id = Session::get('user')->id;
        $Driver->approved_date = Carbon::now()->format('Y/m/d');
        $Driver->cancel_reason = $cancel_reason;
        $Driver->save();
    
        return redirect()->route('employee.franchies.team.rider', $Driver->team_id)->with('success', 'Rider updated successfully.');
    }
